...
Tailwind 构建浅色、留白清晰的工作台。桌面左侧项目列表、中间聊天/视觉标签页、右侧可折叠需求表单；手机使用抽屉与标签页。
...
3D 使用 React Three Fiber，Scene JSON 为场景事实来源。多视角先从同一场景固定相机渲染，独立生成的概念图不可冒充一致的3D实景。
...
Phase1: POST assets/upload-url, assets/:id/analyze；GET scene。
Phase2: POST scene/commands, revisions/:id/revert, exports；GET revisions。
...
未来：add_reference_furniture、update_reference_furniture、remove_reference_furniture；只能使用目录内 asset_id 和受支持参数。
...
网页为工作入口；PDF为3–5页摘要快照；JSON为可追溯数据。不是施工图或BIM成果。

## 部署与运维
先核查现有服务与空闲端口。应用独立 Compose project，不复用 MCP 服务端口。
