<script lang="ts">
  ...
  export function focusBounds(b: { x: number; y: number; width: number; depth: number }) {
    if (!camera || !controls) throw new Error('Viewer is not ready');
    if (walkthroughMode) exitWalkthroughMode();
    const target = new THREE.Vector3(b.x + b.width / 2, activeFloorElevation + 45, b.y + b.depth / 2);
    const distance = Math.max(b.width, b.depth) * 1.12;
    controls.target.copy(target);
    camera.position.set(target.x + distance * 0.75, target.y + distance * 1.8, target.z + distance * 0.65);
    controls.update(); markSceneDirty();
  }
  export function setNavigation(mode: '3d' | 'walk') {
    if (!camera) throw new Error('Viewer is not ready');
    if (mode === 'walk' && !walkthroughMode) enterWalkthroughMode();
    if (mode === '3d' && walkthroughMode) exitWalkthroughMode();
    editMode = embedded && mode !== 'walk'; markSceneDirty();
  }
  export function orbitStep(angle: number) {
    const offset = camera.position.clone().sub(controls.target);
    offset.applyAxisAngle(new THREE.Vector3(0, 1, 0), angle);
    camera.position.copy(controls.target).add(offset); controls.update(); markSceneDirty();
  }
  ...
  function init() {
    ...
    renderer.domElement.addEventListener('pointerup', (e) => {
      ...
      const intersects = raycaster.intersectObjects(wallGroup.children, embedded);
      if (embedded) {
        const hit = intersects.find(hit => hit.object.userData.objectId || hit.object.userData.wallId);
        selectedElementId.set(hit?.object.userData.objectId ?? hit?.object.userData.wallId ?? null);
        return;
      }
  ...
  function buildWalls(floor: Floor) {
    ...
    for (const fi of floor.furniture) {
      ...
      // Demo uses upstream MIT procedural furniture, avoiding unverified third-party model rights.
      const model = embedded ? createFurnitureModel(fi.catalogId, furnitureDef) : createFurnitureModelWithGLB(fi.catalogId, furnitureDef, () => {
